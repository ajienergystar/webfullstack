...
echo "Running pos/integration-tables.sql (migration)..."
/opt/mssql-tools18/bin/sqlcmd -S "$SERVER" -U sa -P "$PASSWORD" -C -i /scripts/pos/integration-tables.sql 2>/dev/null || true

echo "Running pos/backup-tables.sql (migration)..."
/opt/mssql-tools18/bin/sqlcmd -S "$SERVER" -U sa -P "$PASSWORD" -C -i /scripts/pos/backup-tables.sql 2>/dev/null || true
